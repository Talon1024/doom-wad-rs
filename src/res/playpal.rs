...
pub struct PaletteCollection<'wad> {
    lump: &'wad DoomWadLump,
}

// rust-by-example/scope/lifetime/trait.html
impl<'wad> From<&'wad DoomWadLump> for PaletteCollection<'wad> {
    fn from(lump: &'wad DoomWadLump) -> PaletteCollection<'wad> {
        PaletteCollection { lump }
    }
}
...
impl<'wad> PaletteCollection<'wad> {
...
impl<'wad> ToImage for PaletteCollection<'wad> {
